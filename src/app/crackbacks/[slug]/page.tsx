// file: src/app/crackbacks/[slug]/page.tsx
import { notFound } from "next/navigation"
import imageUrlBuilder from "@sanity/image-url"
import { crackbackBySlugQuery } from "@/sanity/lib/queries"
...
export default async function CrackbackPage({
  params,
}: {
  ...
  const post = await client.fetch(crackbackBySlugQuery, { slug })
      ...
      <div className="mx-auto max-w-3xl px-6 pb-24 pt-14">
        ...
        <div className="mb-12">
          <div className="flex flex-wrap items-center gap-3">
            <span className="text-[11px] font-semibold uppercase tracking-[0.22em] opacity-55">
              {formatCrackbackNumber(post.number)}
            </span>

            {post.company ? (
              <span className="text-[11px] font-semibold uppercase tracking-[0.22em] opacity-45">
                {post.company}
              </span>
            ) : null}
          </div>
          ...
          <div className="mt-5 flex flex-wrap items-center gap-x-4 gap-y-2 text-sm opacity-50">
            {post.publishedAt ? (
              <span>
